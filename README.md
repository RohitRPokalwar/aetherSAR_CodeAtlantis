...
|---------|-------------|-------|
| YOLOv8s + ByteTrack | Real-time ship detection & multi-object tracking | ultralytics, OpenCV |
| CFAR Pre-Screening | Radar-native target detection before YOLO | NumPy, OpenCV |
| Soft-NMS | Gaussian/linear decay tile merging | NumPy |
...
| Morphological Post-Processing | Intensity validation + shape filtering | OpenCV, NumPy |
...
| Interactive Metrics | PR curves, F1 analysis, confusion matrix | matplotlib, NumPy |
| Training Dashboard | Fine-tune YOLO from UI with progress tracking | Ultralytics |
...
│   ├── detection/      # YOLOv8 detector, CFAR, Soft-NMS, tracker, classifier, postprocess
│   ├── analytics/      # Threat scoring, zones, dark vessel, fleet, trajectory, metrics
│   ├── visualization/  # Renderer, Folium map, heatmap
│   ├── reporting/      # NL query, PDF/HTML reports
│   └── pipeline.py     # End-to-end orchestration
├── dashboard/          # Streamlit UI (9 pages)
├── scripts/            # Training, evaluation, dataset download
└── requirements.txt
```
...
- Single VV polarization only; dual-pol (VV+VH) analysis could improve accuracy
- No SAR-specific backbone; COCO-pretrained YOLOv8 backbone used
- CFAR parameters may need tuning for different sea states
