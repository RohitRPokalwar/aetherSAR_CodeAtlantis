...
def train(args):
    ...
    results = model.train(
        ...
        plots=True,
        verbose=True,
        workers=0,  # Fixes MemoryError on Windows
    )

    # Copy best weights
    best_weights = MODELS_DIR / "runs" / "yolov8s_sar" / "weights" / "best.pt"
    if best_weights.exists():
        import shutil
        dest = MODELS_DIR / "yolov8s_sar.pt"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train YOLOv8s for SAR ship detection")
    parser.add_argument("--data", type=str, required=True, help="Dataset YAML or directory path")
    parser.add_argument("--model", type=str, default="yolov8s.pt", help="Base model")
